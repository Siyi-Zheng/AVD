...
%cases: [case 1a, case 1b, case 3, case 2]
...
dW_total(1,721) = dW_total(1,721) + dW_engine(1)*0.01;         %add weight of engine 1
dW_total(1,1857) = dW_total(1,1857) + dW_engine(1)*0.01;     %add weight of engine 2
dW_total(1,306) = dW_total(1,306) + dW_mg(1)*0.01;             %add weight of landing gear
dW_total(1,:) = dW_total(1,:) * 100;

%case 1b - worst case scenario with no fuel weight
dW_total(2,:) = dW_total(2,:) + dW_wing;                             %add weight of wing
dW_total(2,721) = dW_total(2,721) + dW_engine(2)*0.01;         %add weight of engine 1
dW_total(2,1857) = dW_total(2,1857) + dW_engine(2)*0.01;     %add weight of engine 2
dW_total(2,306) = dW_total(2,306) + dW_mg(2)*0.01;             %add weight of landing gear
...
dW_total(3,721) = dW_total(3,721) + dW_engine(3)*0.01;         %add weight of engine 1
dW_total(3,1857) = dW_total(3,1857) + dW_engine(3)*0.01;     %add weight of engine 2
dW_total(3,306) = dW_total(3,306) + dW_mg(3)*0.01;             %add weight of landing gear
dW_total(3,:) = dW_total(3,:) * 100;
%dW_total(3,306) = dW_total(3,306) - 6.2349e6;             %add force of main landing gear on the wing at touch down
...
dW_total(4,721) = dW_total(4,721) + dW_engine(2)*0.01;         %add weight of engine 1
dW_total(4,1857) = dW_total(4,1857) + dW_engine(2)*0.01;     %add weight of engine 2
dW_total(4,306) = dW_total(4,306) + dW_mg(2)*0.01;             %add weight of landing gear
...
dL(1 , :) = dL1';                            %case 1a
dL(2 , :) = dL(1,:);                        %case 1b
dL(3 , :) = zeros(1 , length(span));        %case 3
dL(4 , :) = dL2';                            %case 1a
...
%GRAPH FOR THE REPORT (VERTICAL LOADS)
figure
plot(span,dL(1,:)-dW_total(1,:),"r",LineWidth = 1.25);
hold on
...
% %GRAPH FOR THE REPORT (HORIZONTAL LOADS)
dD = 0.5*rho*v.^2.*C_d.*dS;                 %drag distribution
dD = dD * 100;
dT = zeros(4,length(span));
dT(1:3,721) = dT(1:3,721) + T*0.01;         %thrust of engine 1 for cases 1a, 1b and 3
dT(1:3,1857) = dT(1:3,1857) + T*0.01;         %thrust of engine 2 for cases 1a, 1b and 3
dT(4,721) = dT(4,721) + T*0.01;             %only add thrust from engine 1 for worst case scenario


figure
plot(span,dD(1,:)-dT(1,:),"r",LineWidth = 1.25);
hold on
plot(span,dD(4,:)-dT(4,:),"g",LineWidth = 1.25);
plot(span,dD(3,:)-dT(3,:),LineWidth = 1.25,Color=[0 128 255]/255);
plot(span,dD(1,:),"--", LineWidth = 1.25,Color=[255 102 102]/255);
plot(span,dD(4,:),"--", LineWidth = 1.25,Color=[50 153 50]/255);
plot(span,dD(3,:),"--", LineWidth = 1.25,Color=[102 102 255]/255);
plot(span,-dT(1,:),":", LineWidth = 1.25,Color=[153 0 0]/255);
plot(span,-dT(4,:),":", LineWidth = 1.25,Color=[0 50 0]/255);
plot(span,-dT(3,:),":", LineWidth = 1.25,Color=[0 0 153]/255);
hold off
%ylim([-6 2.5]*10e4);
xlabel("Semi-span (m)");
ylabel("Load (N)");
grid on
legend("Case 1 - Resultant load","Case 2 - Resultant load","Case 3 - Resultant load","Case 1 - Drag ","Case 2 - Drag " ,"Case 3 - Drag ","Case 1 - Thrust","Case 2 - Thrust","Case 3 - Thrust");


figure
plot(span,dD(1,:)-dT(1,:),"r",LineWidth = 1.25);
hold on
plot(span,dD(2,:)-dT(2,:),"k",LineWidth = 1.25);
plot(span,dD(4,:)-dT(4,:),"g",LineWidth = 1.25);
plot(span,dD(3,:)-dT(3,:),LineWidth = 1.25,Color=[0 128 255]/255);
plot(span,dD(1,:),"--", LineWidth = 1.25,Color=[255 102 102]/255);
plot(span,dD(2,:),"--", LineWidth = 1.25,Color=[96 96 96]/255);
plot(span,dD(4,:),"--", LineWidth = 1.25,Color=[50 153 50]/255);
plot(span,dD(3,:),"--", LineWidth = 1.25,Color=[102 102 255]/255);
plot(span,-dT(1,:),":", LineWidth = 1.25,Color=[153 0 0]/255);
plot(span,-dT(2,:),":", LineWidth = 1.25,Color=[192 192 192]/255);
plot(span,-dT(4,:),":", LineWidth = 1.25,Color=[0 50 0]/255);
plot(span,-dT(3,:),":", LineWidth = 1.25,Color=[0 0 153]/255);
hold off
%ylim([-6 2.5]*10e4);
...
ylabel("Load (N)");
grid on
legend("Case 1a - Resultant load","Case 1b - Resultant load","Case 2 - Resultant load","Case 3 - Resultant load","Case 1a - Drag ","Case 1b - Drag ","Case 2 - Drag " ,"Case 3 - Drag ","Case 1a - Thrust","Case 1b - Thrust","Case 2 - Thrust","Case 3 - Thrust");
...
ylabel("Load (N/m)");
grid on
legend("Case 1a - Resultant load","Case 1b - Resultant load","Case 2 - Resultant load","Case 3 - Resultant load");
