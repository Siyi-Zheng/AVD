...
momentMax = M_x(1,1); % Nm % maximum bending moment
moment_bottom = M_x(4,1);
percent_moment = (abs(moment_bottom)- momentMax) / momentMax;
...
n_bottom = floor((n * percent_moment) + n) ;
b_bottom = c./n_bottom.*1000;
...
b2_span = 0.097 * chord;
c_span = chord * 0.58;
I = chord .* (t_e ./ 1000) .^ 3 / 12 + chord .* (t_e ./ 1000) .* b2_span .^ 2 / 4;

crush_load = M_x .^ 2 .* rib_spacing .* b2_span .* (t_e ./ 1000) .* chord...
    ./ (2 .* E_T861 .* 1e9 .* (I .^ 2));
t_r = (crush_load ./ chord ./ 3.62 ./ (E_T861 .* 1e9) .* b2_span .^ 2) .^ (1/3) .* 1000;
...
rib_spacing_test = 0.2:0.05:1.6; % m %vary this after iterations
...
t_2_span_disc_bottom = (t2_span_disc .* percent_moment) + t2_span_disc;
t_2_span_disc_bottom(t_2_span_disc_bottom<1)=1;
...
skin_stringer_fmass = ((density_T861) .* volume_ss);

t_e_span_b = t_2_span_disc_bottom + As ./ b_bottom;
[unique_t_e_b, stations] = unique(t_e_span_b);
stations_r = flip(stations);
span_ss= span(stations_r);
l_y_ss= (0.7-0.12).*chord(stations_r);

for j=1:length(l_y_ss)
    if j == length(l_y_ss)
        area_ss_b(j)= 0.5*(l_y_ss(j) + (0.7-0.12)*chord(end)) *(span(end)-span_ss(j));
    else
        area_ss_b(j) = 0.5* (l_y_ss(j) + l_y_ss(j+1)) * (span_ss(j+1)-span(j));
    end
end

volume_ss_b = sum(area_ss_b.* (unique_t_e_b./1000)); %m^3
skin_stringer_fmass_b = ((density_T861) .* volume_ss_b);
...
% new spar mass calculations
spar_angle = 1/cosd(30.3);
spar_area_f = b2_span .* t_FS_dis * 1.4; % consider flanges
spar_area_r = b2_span .* t_RS_dis * 1.4; % consider flanges
mass_fs = trapz(span, spar_area_f) * density_H34 * 1e-3;
mass_rs = trapz(span, spar_area_r) * density_H34 * 1e-3;

mass_fuck = ribs_fmass + skin_stringer_fmass + spar_mass + skin_stringer_fmass_b
disp(mass_fuck)
%% 
...
% 
% % d-cell design
% perimeter = 0.1739 * chord;
% area = 0.243 / 100 * chord .^ 2;
% min_pseudoribs = 5;
% max_pseudoribs = 20;
% root_radius = 0.3; % m, this can be anything lol
% radius = root_radius .* chord / chord(1);
% buckling_coeff = [];
% total_mass = [];
...
% % Set the color of each axis to bla1ck
...
mass_fuck = ribs_fmass + skin_stringer_fmass + spar_mass + skin_stringer_fmass_b + total_mass(end);
disp(mass_fuck)
...
% new code - use optimal number of stringers
[min_t_e, opt_n] = min(t_e_span);
