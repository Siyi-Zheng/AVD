...
H = atan(deg2rad(gamma))*(L_plane-x_mg-200/39.37); % correction for the rear main gear
...
% Main Wheel
%Three-part Name Tire: 37 x 14.0-14
R_r = 21.3; % Rolling Radius in INCH
w_tire = 20.5;
d_tire = 52;
...
%Tire Pressure
A_p = 2.3*sqrt(w_tire*d_tire)*(d_tire/2 - R_r); %wheel contact area in inch^2
P_mw = W_mw_lbs/A_p; % tire pressure in PSI
...
S_T = D_mw/2 - R_r; % stroke of tire (half radius - rolling radius)
...
% Main wheel
W_moleo = W_mg*9.81/4; % Load on wheel in Newton
